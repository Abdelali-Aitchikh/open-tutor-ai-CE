...
{#if loaded}
	...
	>
		...
		<div class="{transparentBackground ? 'bg-transparent' : 'bg-[#F5F7F9] dark:bg-gray-900'} ">
			<div
			...
			>
				<div class="">
					...
					{:else}
						...
						>
							<div
								class="flex-1 flex flex-col relative w-full rounded-4xl {transparentBackground
									? 'shadow-xl shadow-black/20 dark:shadow-black/40 bg-white/40 backdrop-blur-md dark:bg-gray-800/60 border-white/40 dark:border-gray-600/50'
									: 'shadow-md bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700'} dark:text-gray-100 border px-6 py-2 max-w-3xl mx-auto mb-10 {transparentBackground
									...
									<div class="flex-1 min-w-0">
										...
										{:else}
											<textarea
												id="chat-input"
												bind:this={chatInputElement}
												class="scrollbar-hidden bg-transparent dark:text-gray-100 outline-hidden w-full resize-none max-h-80 overflow-y-auto break-words"
												placeholder={placeholder ? placeholder : $i18n.t('Send a Message')}
												bind:value={prompt}
												on:keypress={(e) => {
													if (
													...
													<button
														...
														class="{!(prompt === '' && files.length === 0)
															? webSearchEnabled || ($settings?.webSearch ?? false) === 'always'
																...
																: 'bg-blue-500 text-white hover:bg-blue-400 dark:bg-blue-400 dark:text-white dark:hover:bg-blue-300'
															: 'text-white bg-gray-200 dark:text-gray-900 dark:bg-gray-700 disabled'} transition rounded-full p-1.5"
														type="submit"
